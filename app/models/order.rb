class Order < ActiveRecord::Base
  ...
  state_machine initial: :pending do # 默认状态，待付款
    ...
    after_transition :paid => :shipping do |order, transition|
      WXMessageJob.perform_later(order.user.id, SiteConfig.order_state_msg_tpl, '', { first: '订单配送中，请耐心等待:)', remark: '', values: ["#{order.order_no}", "已发货"] })
    ...
    after_transition [:pending, :paid] => :canceled do |order, transition|
      ...
      WXMessageJob.perform_later(order.user.id, SiteConfig.order_state_msg_tpl, '', { first: '系统人工取消了您的订单', remark: '', values: ["#{order.order_no}", "已取消"] })
    ...
    after_transition :shipping => :completed do |order, transition|
      ...
      WXMessageJob.perform_later(order.user.id, SiteConfig.order_state_msg_tpl, '', { first: '您已经完成了本次购物，谢谢惠顾！', remark: '欢迎到微信跟我们留言互动，多提意见！我们会加倍努力的哟:)', values: ["#{order.order_no}", "已完成"] })
